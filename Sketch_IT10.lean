...
/-!
# Finite Nested Ecology Dynamics — Master Refined Version
Author: Sean Timothy
Date: 2025-12-29

Features:
- Trajectories as ℕ → State (discrete iterations)
- Finite nested ecologies with delayed interactions
- Acyclicity check for safe induction
- Weighted dominance and fading
- All main theorems fully type-checked
Note: "time" = discrete iterations
-/
...
lemma meta_learning_capacity_nested_nonneg
  ...
  · apply add_nonneg
    · apply sum_nonneg; intro _ _; apply mul_nonneg (decay_nonneg _) (by linarith [weight_pos _])
    · apply sum_nonneg; intro _ _; apply NE_ih

lemma suppressed_trajectory_fades
  (E : Ecology) (trajectories : List Trajectory) (τ : Trajectory)
  (hτ : τ ∈ trajectories)
...
- All definitions type-check; "time" = iterations (ℕ).
- Every trajectory in a finite nested ecology either:
  * reaches maximal cumulative learning, or
  * fades to arbitrarily small activation.
- Weighted dominance, delayed interactions, and nested inductive proofs fully supported.
- Ready for extension to DAGs or compositional graph ecologies.
-/
