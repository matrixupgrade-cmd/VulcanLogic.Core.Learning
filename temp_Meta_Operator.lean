...
-- ────────────────────────────────────────────────────────────────
-- 0. Assumed primitives (from prior modules or instantiation)
-- ────────────────────────────────────────────────────────────────
...
by
  ...
  rw [this]
  -- Time starts at 1 on first observation (history.length + 1)
  have : obs.time ≥ 1 := by simp
...
by
  ...
  intro h hh
  exact (h_hor h hh).1.1.nonempty  -- Nonempty witness from harmony condition

-- Extract bounded drift from strict harmony
...
-- ────────────────────────────────────────────────────────────────
-- 6. Operational semantics (history generation)
-- ────────────────────────────────────────────────────────────────
...
by
  ...
  -- From harmonious_strict on recent fingerprints
  have h_strict : harmonious_strict basin obs.fingerprint := sorry  -- extract from hharmony + h_mem
  exact harmonious_strict_implies_bounded_drift basin obs.fingerprint h_strict
...
def cultivate_step (meta : MetaOperator) (basin : Basin) (history : History) : MetaOperator × History :=
  let fallback := some "safer intervention"  -- placeholder for conservative action
...
theorem meta_operator_eventually_stabilizes_viability
  ...
  ∃ horizon ≤ max_iters,
    ∀ steps ≥ horizon,
      let (meta', h') := iterate (λ p => cultivate_step meta₀ basin p.2) (meta₀, []) steps
      let last_fp := (h'.getLast?.map Observation.fingerprint).getD default
      bounded_drift last_fp ∧ viability last_fp ≥ 0 :=
by
  admit  -- proof sketch: harmony bounds stress frequency; fallback reduces high-stress zones over time
